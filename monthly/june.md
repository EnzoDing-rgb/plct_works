...
- 提出对标 MIT 6.5940 的 8 周 TinyML KWS 课程蓝图：预训练（数据采集 + MFCC + DS-CNN）→ 后训练（PTQ/量化/剪枝）→ 推理部署（int8 C + LED 控灯），详见 [`tinyml-kws-course-blueprint.md`](https://github.com/EnzoDing-rgb/riscv-embedded-course/blob/master/docs/archive/tinyml-kws-course-blueprint.md)。
